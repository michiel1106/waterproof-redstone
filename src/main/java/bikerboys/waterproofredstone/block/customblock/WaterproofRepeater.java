...
public class WaterproofRepeater extends AbstractRedstoneGateBlock implements Waterloggable{
    public static final BooleanProperty WATERLOGGED;

    public static final BooleanProperty LOCKED;
    public static final IntProperty DELAY;

    public WaterproofRepeater(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with(FACING, Direction.NORTH)).with(DELAY, 1)).with(LOCKED, false)).with(POWERED, false).with(WATERLOGGED, false));
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        // Set the LOCKED and WATERLOGGED properties in the block state
        return blockState
                .with(LOCKED, this.isLocked(ctx.getWorld(), ctx.getBlockPos(), blockState))
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(new Property[]{FACING, DELAY, LOCKED, POWERED, WATERLOGGED});
    }
